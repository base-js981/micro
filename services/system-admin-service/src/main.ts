...
async function bootstrap(): Promise<void> {
  ...
  const config = new DocumentBuilder()
    .setTitle('System Administration Service API')
    .setDescription('System Administration Service API')
    .setVersion('1.0')
    .addTag('system-admin', 'System administration endpoints')
  ...
  const kafkaClientId = configService.get<string>('KAFKA_CLIENT_ID', 'system-admin-service');
  ...
  const grpcPort = configService.get<number>('GRPC_PORT', 50055);
  const protoPath = getProtoPath('services/system-admin-service.proto');

  app.connectMicroservice<MicroserviceOptions>({
    transport: Transport.GRPC,
    options: {
      package: 'system.admin.service',
      protoPath,
