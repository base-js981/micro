...
async function bootstrap(): Promise<void> {
  ...
  const config = new DocumentBuilder()
    ...
    .setDescription('Integration Service API')
    .setVersion('1.0')
    .addTag('integration', 'System administration endpoints')
  ...
  const httpPort = configService.get<number>('PORT', 3008);
  await app.listen(httpPort);

  const kafkaBrokers = configService.get<string>('KAFKA_BROKERS', 'localhost:9092').split(',');
  const kafkaClientId = configService.get<string>('KAFKA_CLIENT_ID', 'integration-service');
  ...
  const grpcPort = configService.get<number>('GRPC_PORT', 50058);
  const protoPath = getProtoPath('services/integration-service.proto');
