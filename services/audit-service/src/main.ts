...
async function bootstrap(): Promise<void> {
  ...
  const config = new DocumentBuilder()
    .setTitle('Audit & Logging Service API')
    .setDescription('Audit & Logging Service API')
    .setVersion('1.0')
    .addTag('audit', 'System administration endpoints')
  ...
  const httpPort = configService.get<number>('PORT', 3009);
  await app.listen(httpPort);

  const kafkaBrokers = configService.get<string>('KAFKA_BROKERS', 'localhost:9092').split(',');
  const kafkaClientId = configService.get<string>('KAFKA_CLIENT_ID', 'audit-service');
  ...
  const grpcPort = configService.get<number>('GRPC_PORT', 50059);
  const protoPath = getProtoPath('services/audit-service.proto');
